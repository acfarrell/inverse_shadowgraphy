...
def initialize():
    ...
    if len(sys.argv) != 2:
        ...
        # Get the output directory to save to
        try:
        ...
        try:
            os.mkdir(output_dir)
            log.info(f'All output will be saved to the new directory {output_dir}')
        except OSError:
            log.info(f'All output will be saved to the existing directory {output_dir}')
        ...
        return params, output_dir, suffix, save_txt
    
    
def save_run(output_dir, suffix, save_txt, results):
    '''Save all the data for this run'''
    fname = output_dir + 'output' + suffix + '.npz'
    log.info(f'Saving run data to {fname}')
    np.savez(fname, **results)
    return

def main():
    params, output_dir, suffix, save_txt = initialize()
    target, phi_c, alpha_x, alpha_y = ot.get_deflection_potential(**params)
    ...
    plt.savefig(output_dir+'site_displacement'+suffix+'.png', 
                dpi=300, 
                bbox_inches='tight')
    save_run(output_dir, suffix, save_txt, results)
    return
